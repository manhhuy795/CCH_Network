...
def test_runtime_topology_declares_new_trunks_and_router_subinterfaces():
    ...
    assert '"access_floor1": ("dist_hq_1", "f1-eth99", "d1-eth01", [20, 30, 110, 120])' in source
    assert '"access_floor2": ("dist_hq_2", "f2-eth99", "d2-eth01", [30, 40, 60, 70])' in source
    assert 'switches["access_branch"].cmd("ovs-vsctl set port br-eth99 vlan_mode=trunk trunks=40,50,111")' in source
    assert 'switches["dist_branch"].cmd("ovs-vsctl set port bd-eth40 tag=40")' in source
