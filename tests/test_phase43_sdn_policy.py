...
def test_phase43_reactive_forwarding_preserves_ovs_vlan_pipeline():
    controller = CONTROLLER_PATH.read_text(encoding="utf-8")
    method = controller.split("def packet_in_handler", 1)[1].split("def ", 1)[0]
    assert 'normal_port = getattr(ofproto, "OFPP_NORMAL", 0xFFFFFFFA)' in method
    assert "actions = [parser.OFPActionOutput(normal_port)]" in method
    assert "parser.OFPActionOutput(out_port)" not in method


def test_phase43_runtime_routing_uses_live_mpls_and_firewall_interface_names():
    topology = TOPOLOGY_PATH.read_text(encoding="utf-8")
    assert '"mpls-p-eth0"' in topology
    assert '"mpls-p-eth1"' in topology
    assert '"mpls-b-eth0"' in topology
    assert '"mpls-b-eth1"' in topology
    assert 'add_route(fw_hq, "10.255.30.0/24", "10.255.10.2")' in topology
    assert 'add_route(internet_zone, prefix, "10.255.10.1")' in topology
    assert 'add_route(fw_hq, vip, "10.255.10.2")' in topology
    assert 'internet_zone.cmd(f"ip route replace {vip} dev inet-eth2")' in topology
    assert 'if len(interface) > 15:' in topology
    assert 'interface = f"vlan{int(vlan)}"' in topology
    assert 'disable_mpls_primary_routes(self.net)' in topology
    assert 'remove_route(ce_telesale, prefix, "10.255.0.5", metric=10)' in topology
