...
def test_hq_floor_placement_matches_enterprise_design():
    ...
    assert {hosts[f"h40_{i:02d}"]["switch"] for i in range(1, 11)} == {"access_floor2"}
    assert {hosts[f"h40_{i:02d}"]["switch"] for i in range(11, 21)} == {"access_branch"}
    assert {hosts[f"h40_{i:02d}"]["site"] for i in range(1, 11)} == {"hq"}
    assert {hosts[f"h40_{i:02d}"]["site"] for i in range(11, 21)} == {"branch_telesale"}
