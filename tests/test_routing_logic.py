...
def test_two_ce_mpls_routes_use_direct_provider_handoffs_and_metrics():
    ...
    assert any(route["next_hop"] == "10.255.0.6" for route in routes["mpls_primary"]["routes"])
    assert any(route["next_hop"] == "10.255.0.14" for route in routes["mpls_backup"]["routes"])
...
def test_vlan60_is_local_to_hq_and_remote_from_telesale():
    ...
    branch_mpls_prefixes = {route["prefix"] for route in load_vars()["ce_telesale"]["mpls_routes"]}
    assert "172.16.60.0/24" in branch_mpls_prefixes
    assert "172.16.40.0/24" not in branch_mpls_prefixes
