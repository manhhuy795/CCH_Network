...
def test_generic_link_recovery_updates_runtime_state(monkeypatch):
    module = _load_topology_module(monkeypatch)
    ...
    class FakeNet:
        def __init__(self):
            self.nameToNode = {
                "core_hq": FakeNode("core_hq"),
                "fw_hq": FakeNode("fw_hq"),
            }
            self.link_changes = []
    ...
    control_agent = module.MininetControlAgent(
        net,
        {"links": [["core_hq", "fw_hq", "routed"]], "host_groups": {}},
        Path("unused-test.sock"),
        "test-token",
    )
    ...
    result = control_agent._set_link("core_hq-fw_hq", "up")

    assert result["ok"] is True
    assert result["status"] == "up"
    assert net.link_changes == [("core_hq", "fw_hq", "up")]
