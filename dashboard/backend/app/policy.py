...
POLICY_CATALOG: dict[str, dict[str, Any]] = {
    "isolate_hq_projects": {
        "name": "Cô lập các dự án HQ",
        "description": "Chặn lưu lượng ngang giữa Dự án 1, 2, 3 và 4 theo source enforcement switch.",
    ...
    "allow_voice": {
        ...
        "description": "Cài flow hai chiều cho các VLAN được quản lý truy cập Partner PBX/Contact Center.",
        "source": "VLAN 101 / 93 / 103 / 104 / 110",
        "destination": "h90 · Partner PBX 10.250.10.10/32",
        "action": "ALLOW",
    ...
    "allow_guest_general_internet": {
        "name": "Cho phep Guest ra Internet",
        "description": "Guest chi duoc breakout Internet va khong duoc truy cap mang noi bo.",
        "source": "VLAN 120 / Guest",
        "destination": "hinternet",
        "action": "ALLOW",
        "enforcement_point": "fw_hq (nftables)",
        "priority": 0,
        "cookie": "n/a",
    },
    "block_social_media": {
        ...
        "source": "Managed user VLAN / VLAN 110 IT Support",
        "destination": "hsocial",
        "action": "DROP",
        "enforcement_point": "fw_hq / fw_telesale (nftables)",
        "priority": 0,
    ...
    "voice_flow_priority": {
        ...
        "description": "Tăng OpenFlow priority của luồng Voice lên 425; không tuyên bố DSCP/queue QoS production.",
        "source": "User VLAN được cấp quyền",
        "destination": "h90 · Partner PBX 10.250.10.10/32",
        "action": "ALLOW",
        "enforcement_point": "Source enforcement switch",
...
def get_policy_payload() -> dict:
    ...
    payload["enforcement_layers"] = {
        "openflow": {
            ...
            "responsibilities": [
                "Cô lập Dự án 1/2/3/4 tại source enforcement switch",
                "Cài flow hai chiều tới Partner PBX h90",
                "IT Support VLAN 110 truy cập có kiểm soát",
            ],
        },
        "nftables": {
